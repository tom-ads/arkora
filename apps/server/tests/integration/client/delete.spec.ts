...
test.group('Clients : Delete', ({ each }) => {
  let authUser: User
  ...
  test('organisation org_admin can delete a client', async ({ client, route }) => {
    const orgAdminRole = await RoleFactory.apply('orgAdmin').create()
    await authUser.related('role').associate(orgAdminRole)

    const response = await client
      .delete(route('ClientController.delete', { clientId: orgClient.id }))
  ...
  test('organisation owner can delete a client', async ({ client, route }) => {
    const response = await client
      .delete(route('ClientController.delete', { clientId: orgClient.id }))
      .headers({ origin: `http://test-org.arkora.co.uk` })
      .withCsrfToken()
  ...
  test('organisation manager cannot delete a client', async ({ client, route }) => {
  ...
  test('organisation member cannot delete a client', async ({ client, route }) => {
  ...
  test('unauthenticated user cannot delete a client', async ({ client, route }) => {
    const response = await client
      .delete(route('ClientController.delete', { clientId: orgClient.id }))
      .headers({ origin: `http://test-org.arkora.co.uk` })
      .withCsrfToken()
  ...
  test('organisation user cannot delete another organisations client', async ({
    client,
    route,
  }) => {
